...
//--  !!!NOT NEEDED        if (webphone_api.common.GetParameterBool('customizedversion', true) === false) { return; }
        ...
        var chatsms = webphone_api.common.GetParameter('chatsms');
        ...
        if (webphone_api.global.mto.indexOf(',') > 0)
        {
            ...
            for (var i = 0; i < dstlist.length; i++)
            {
                ...
                if (sendrec === false)
                {
                    ...
                    webphone_api.common.UriParser(webphone_api.common.GetParameter('sms'), '', webphone_api.common.GetSipusername(), webphone_api.common.Trim(dstlist[i]), chatwith, 'sendsms');
                }
                
                webphone_api.common.UriParser(webphone_api.common.GetParameter('sms'), '', webphone_api.common.GetSipusername(), webphone_api.common.Trim(dstlist[i]), msgLocal, 'sendsms');
            }
        }else
        {
            webphone_api.common.UriParser(webphone_api.common.GetParameter('sms'), '', webphone_api.common.GetSipusername(), toLocal, msgLocal, 'sendsms');
        }
...
// if received chat message is filetransfer, then create a link from url
    if (mMessage.indexOf('/filestorage/') > 0)
    {
        ...
        if (pos >= 0)
        {
            ...
            if (pos > 0) { furl = webphone_api.common.Trim(furl.substring(0, pos)); }
