...
// attach click listeners   conference,transfer,numpad,mute,hold,speaker
    ...
    webphone_api.$('#btn_transfer').on('click', function(event) { CallfunctionsOnclick('transfer'); });
...
{
    try{
    webphone_api.common.PutToDebugLog(1, 'EVENT, ' + webphone_api.stringres.get('initiate_call_transfer'));
    

//--    if (webphone_api.common.getuseengine() === webphone_api.global.ENGINE_WEBRTC && webphone_api.common.GetParameter('transf_engineswitcheoffered') !== 'true')
//--    {
//--        var ep_webrtc = webphone_api.common.StrToInt(webphone_api.common.GetParameter2('enginepriority_webrtc'));
//--        var ep_java = webphone_api.common.StrToInt(webphone_api.common.GetParameter2('enginepriority_java'));
//--        var ep_ns = webphone_api.common.StrToInt(webphone_api.common.GetParameter2('enginepriority_ns'));
...
//--        if (ep_ns > 0 && ep_webrtc - ep_ns < 3 && webphone_api.common.CanIUseService() === true)
//--        {
//--            webphone_api.common.EngineSwitchTransfer('ns', phoneNr, Transfer);
//--            return;
//--        }
//--        if (ep_java > 0 && ep_webrtc - ep_java < 3 && webphone_api.common.CanIUseApplet() === true)
//--        {
//--            webphone_api.common.EngineSwitchConference('java', phoneNr, Transfer);
//--            return;
//--        }
//--    }
...
//--            {
//--                var nr = webphone_api.global.ep[i][webphone_api.common.EP_DESTNR];
//--                if (!webphone_api.common.isNull(nr) && nr.length > 0 && nr !== callnumber)
//--                {
//--                    webphone_api.transfer(nr, webphone_api.global.aline);
//--                    webphone_api.common.SaveParameter('last_transfer_number', nr);
//--                    webphone_api.common.PutToDebugLog(2, 'EVENT, _call: Transfer, multiline transfer: ' + nr);
...
//--    }
    ...
    if ( !webphone_api.common.isNull(popupWidth) && webphone_api.common.IsNumber(popupWidth) && popupWidth > 100 )
    {
        popupWidth = Math.floor(popupWidth / 1.1);
    }else
    {
        popupWidth = 220;
    }
    ...
    {
        interconnectHtml = '<br />'+
            '<button id="btn_interconnect_transfer" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b" style="font-size: .9em">' + webphone_api.stringres.get('interconnect') + '</button>';
    }
    ...
    '<div data-role="header" data-theme="b">' +
        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' + webphone_api.stringres.get('transfer_title') + '</h1>' +
    '</div>' +
    '<div role="main" class="ui-content adialog_content adialog_btn_nexttoinput">' +
        '<span>' + webphone_api.stringres.get('phone_nr') + '</span>' +
        '<div style="clear: both;"><!--//--></div>' +
    ...
    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="javascript:;" id="adialog_quick_transfer" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_3button" data-rel="back" data-transition="flow" title="' + webphone_api.stringres.get('quicktransf_hint') + '">' + webphone_api.stringres.get('btn_quicktransf') + '</a>' +
        '<a href="javascript:;" id="adialog_attended_transfer" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_3button" data-rel="back" data-transition="flow" title="' + webphone_api.stringres.get('attendedtransf_hint') + '">' + webphone_api.stringres.get('btn_attendedtransf') + '</a>' +
        '<a href="javascript:;" id="adialog_negative" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_3button" data-rel="back">' + webphone_api.stringres.get('btn_cancel') + '</a>' +
    '</div>' +
'</div>';
...
//--    webphone_api.$.mobile.activePage.append(template).trigger("pagecreate");
    ...
    webphone_api.$.mobile.activePage.find(".messagePopup").popup().popup("open").bind(
    {
        ...
        {
            webphone_api.$(this).unbind("popupafterclose").remove();
            webphone_api.$('#adialog_quick_transfer').off('click');
            webphone_api.$('#adialog_attended_transfer').off('click');
            webphone_api.$('#adialog_negative').off('click');
            webphone_api.$('#btn_pickct').off('click');
            webphone_api.$('#btn_interconnect_transfer').off('click');
            popupafterclose();
        }
    });
...
//-- listen for enter onclick, and click OK button
//-- no need for this, because it reloads the page
//--    webphone_api.$( "#transfer_popup" ).keypress(function( event )
//--    {
//--        if ( event.which === 13 )
//--        {
//--            event.preventDefault();
...
//--    });
    ...
    webphone_api.$('#adialog_quick_transfer').on('click', function (event)
    {
        webphone_api.common.PutToDebugLog(5,'EVENT, call Transfer unattended(quick) ok onclick');
    ...
    webphone_api.$('#adialog_attended_transfer').on('click', function (event)
    {
        webphone_api.$( '#transfer_popup' ).on( 'popupafterclose', function( event )
        {
            webphone_api.common.PutToDebugLog(5,'EVENT, call Transfer attended ok onclick');

            var textboxval = webphone_api.common.Trim(textBox.value);
            
            if (!webphone_api.common.isNull(textboxval) && textboxval.length > 0)
            {
                //webphone_api.global.is_attended_transfer = true;
                //webphone_api.transfer(textboxval, 2);
                webphone_api.call(textboxval);
    ...
    webphone_api.$('#btn_pickct').on('click', function (event)
    {
        ...
        webphone_api.$( '#transfer_popup' ).on( 'popupafterclose', function( event )
        {
            webphone_api.$( '#transfer_popup' ).off( 'popupafterclose' );

            webphone_api.common.PickContact(Transfer);
        });
    });

    webphone_api.$('#btn_interconnect_transfer').on('click', function (event)
    {
        webphone_api.common.PutToDebugLog(2, 'EVENT, transfer interconnect on click')
    ...
    } catch(err) { webphone_api.common.PutToDebugLogException(2, '_call: Transfer', err); }
}

function AttendedPopup(number) // popup
{
    try{
    var popupWidth = webphone_api.common.GetDeviceWidth();
    if ( !webphone_api.common.isNull(popupWidth) && webphone_api.common.IsNumber(popupWidth) && popupWidth > 100 )
    ...
    var template = '' +
    '<div id="attended_popup" data-role="popup" class="ui-content messagePopup" data-overlay-theme="a" data-theme="a" style="max-width:' + popupWidth + 'px; width: ' + popupWidth + 'px;">' +

    '<div data-role="header" data-theme="b">' +
//        '<a href="javascript:;" data-role="button" data-icon="delete" data-iconpos="notext" class="ui-btn-right closePopup">Close</a>' +
        '<h1 class="adialog_title">' + webphone_api.stringres.get('attended_transfer_title') + '</h1>' +
    '</div>' +
//    '<div role="main" class="ui-content adialog_content adialog_btn_nexttoinput">' +
//    '</div>' +
    '<div data-role="footer" data-theme="b" class="adialog_footer">' +
        '<a href="javascript:;" id="adialog_attended_transfer" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_3button" data-rel="back" data-transition="flow">' + webphone_api.stringres.get('btn_transfer') + '</a>' +
        '<a href="javascript:;" id="adialog_attended_revert" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_3button" data-rel="back" data-transition="flow">' + webphone_api.stringres.get('btn_revert') + '</a>' +
        '<a href="javascript:;" id="adialog_attended_hangup" class="ui-btn ui-corner-all ui-shadow ui-btn-inline ui-btn-b adialog_3button" data-rel="back">' + webphone_api.stringres.get('btn_hangup') + '</a>' +
    '</div>' +
'</div>';
...
//--    webphone_api.$.mobile.activePage.append(template).trigger("pagecreate");
    ...
    webphone_api.$.mobile.activePage.find(".messagePopup").popup().popup("open").bind(
    {
        popupafterclose: function ()
        {
            webphone_api.$(this).unbind("popupafterclose").remove();
            webphone_api.$('#adialog_attended_transfer').off('click');
            webphone_api.$('#adialog_attended_revert').off('click');
            webphone_api.$('#adialog_attended_hangup').off('click');
            popupafterclose();
        }
    });
...
//--    });
    ...
    {
        //webphone_api.$.mobile.activePage.find(".messagePopup").popup("close");
        webphone_api.$( '#attended_popup' ).on( 'popupafterclose', function( event )
        {
            webphone_api.$( '#attended_popup' ).off( 'popupafterclose' );

            webphone_api.common.PutToDebugLog(5,'EVENT, call AttendedPopup: transfer onclick: ' + number);
            webphone_api.transfer(number, 2);
        });
    });

    webphone_api.$('#adialog_attended_hangup').on('click', function (event)
    {
        webphone_api.common.PutToDebugLog(5,'EVENT, call AttendedPopup: hangup onclick');

        webphone_api.hangup();
    });

    webphone_api.$('#adialog_attended_revert').on('click', function (event)
    {
        webphone_api.common.PutToDebugLog(5,'EVENT, call AttendedPopup: revert onclick');
    });
    } catch(err) { webphone_api.common.PutToDebugLogException(2, '_call: AttendedPopup', err); }
}
