

var peer = 0;

function CreateDatatble() {

    $(document).ready(function () {
        var url = 'http://192.168.225.81:4110/api/asterriskSearchApi?extNo=' + window.sessionStorage.getItem("extNo");
        var source =
        {
            dataType: "json",
            dataFields: [
                { name: 'id', type: 'string' },
                { name: 'date', type: 'string' },
                { name: 'src', type: 'string' },
                { name: 'dst', type: 'string' },
                { name: 'duration', type: 'string' },
                { name: 'accountcode', type: 'string' }
            ],
            id: 'id',
            url: url
        };

        var count_1 = 0;
        var count_2 = 0;
        var count_3 = 0;
        var count_4 = 0;
        var count_5 = 0;


        var dataAdapter = new $.jqx.dataAdapter(source);

        

            $("#dataTablePersonProfile").jqxDataTable(
                {
                    width: window.innerWidth - 100,
                    pageable: true,
                    pagerButtonsCount: 10,
                    source: dataAdapter,
                    columnsResize: true,
                    columns: [

                        {
                            text: 'id', dataField: 'id', width: '20%', cellsRenderer: function (row, column, value, rowData) {

                                var image = ''

                                if (count_1 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_1++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_1++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.id
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'date', dataField: 'date', width: '17%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''

                                if (count_2 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.date
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'src', dataField: 'src', width: '15%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''

                                if (count_3 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                }
                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.src
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'dst', dataField: 'dst', width: '14%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''

                                if (count_4 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                }
                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.dst
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'duration', dataField: 'duration', width: '15%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''

                                if (count_5 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                }
                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.duration
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'accountcode', dataField: 'accountcode', width: '20%', cellsRenderer: function (row, column, value, rowData) {
                                var imgUrlCorrect = "assets/img/icons8-checkmark-64.png"

                                var imgUrlWA = "assets/img/icons8-id-not-verified-48.png"

                                var imgUrlNA = "assets/img/icons8-no-entry-30.png"

                                var imgUrlAddNotes = "assets/img/icons8-strikethrough-30.png"

                                var imgUrlblacklist = "assets/img/icons8-delete-user-male-50.png"

                                var imgCorrect = '<img width="30" height="30" style="display: block;" src="' + imgUrlCorrect + '"/>';
                                var imgWA = '<img width="30" height="30" style="display: block;" src="' + imgUrlWA + '"/>';
                                var imgNA = '<img width="30" height="30" style="display: block;" src="' + imgUrlNA + '"/>';
                                var imgNotes = '<img width="20" height="20" style="display: block;" src="' + imgUrlAddNotes + '"/>';

                                var imgBlacklist = '<img width="20" height="20" style="display: block;" src="' + imgUrlblacklist + '"/>';

                                var image = "<div class='row' style='margin-left:5px;'>" + imgCorrect + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgWA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNotes + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgBlacklist + "</div>"
                                return image;
                            }
                        },







                    ]
                });
        

       var sourceDatatable  =
        {
            dataType: 'json',
            type: 'GET',
            dataFields: [
                { name: 'id', type: 'string' },
                { name: 'date', type: 'string' },
                { name: 'src', type: 'string' },
                { name: 'dst', type: 'string' },
                { name: 'duration', type: 'string' },
                { name: 'accountcode', type: 'string' }
            ],
            id: 'id',
            url: 'http://192.168.225.81:4110/api/asterriskSearchApi?extNo=' + window.sessionStorage.getItem("extNo")
        };

       var dataAdapterAst = new $.jqx.dataAdapter(sourceDatatable);

         count_1 = 0;
         count_2 = 0;
         count_3 = 0;
         count_4 = 0;
         count_5 = 0;

        
            $("#dataTable200").jqxDataTable(
                {
                    width: window.innerWidth - 100,
                    pageable: true,
                    pagerButtonsCount: 10,
                    source: dataAdapterAst,
                    columnsResize: true,
                    columns: [
                        {
                            text: 'id', dataField: 'id', width: '20%', cellsRenderer: function (row, column, value, rowData) {

                                var image = ''



                                if (count_1 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_1++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_1++;
                                }


                               
                                image += rowData.id
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'date', dataField: 'date', width: '20%', cellsRenderer: function (row, column, value, rowData) {

                                var image = ''

                                if (count_2 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.date
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'src', dataField: 'src', width: '16%', cellsRenderer: function (row, column, value, rowData) {

                                var image = ''


                                if (count_3 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.src
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'dst', dataField: 'dst', width: '11%', cellsRenderer: function (row, column, value, rowData) {

                                var image = ''


                                if (count_4 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                }
                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.dst
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'duration', dataField: 'duration', width: '11%', cellsRenderer: function (row, column, value, rowData) {

                                var image = ''



                                if (count_5 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                }
                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.duration
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'Status', dataField: 'accountcode', width: '22%', cellsRenderer: function (row, column, value, rowData) {

                                var imgUrlCorrect = "assets/img/icons8-checkmark-64.png"

                                var imgUrlWA = "assets/img/icons8-id-not-verified-48.png"

                                var imgUrlNA = "assets/img/icons8-no-entry-30.png"

                                var imgUrlAddNotes = "assets/img/icons8-strikethrough-30.png"

                                var imgUrlblacklist = "assets/img/icons8-delete-user-male-50.png"

                                var imgCorrect = '<img width="30" height="30" style="display: block;" src="' + imgUrlCorrect + '"/>';
                                var imgWA = '<img width="30" height="30" style="display: block;" src="' + imgUrlWA + '"/>';
                                var imgNA = '<img width="30" height="30" style="display: block;" src="' + imgUrlNA + '"/>';
                                var imgNotes = '<img width="20" height="20" style="display: block;" src="' + imgUrlAddNotes + '"/>';

                                var imgBlacklist = '<img width="20" height="20" style="display: block;" src="' + imgUrlblacklist + '"/>';

                                var image = "<div class='row' style='margin-left:5px;'>" + imgCorrect + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgWA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNotes + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgBlacklist + "</div>"
                                return image;
                            }
                        }

                    ]
                });

        
       

    });

}

function CreateDatatbleHome() {

    $(document).ready(function () {
        var url = 'http://192.168.225.81:4110/api/asterriskSearchApi?extNo=' + window.sessionStorage.getItem("extNo");
        var source =
        {
            dataType: "json",
            dataFields: [
               
                { name: 'date', type: 'string' },
                { name: 'src', type: 'string' },
                { name: 'dst', type: 'string' },
                { name: 'duration', type: 'string' },
                { name: 'accountcode', type: 'string' }
            ],
            id: 'date',
            url: url
        };
        var employeesAdapter = new $.jqx.dataAdapter(source);

        var count_1 = 0;
        var count_2 = 0;
        var count_3 = 0;
        var count_4 = 0;
        var count_5 = 0;
        var descriptionCount = 0;
        

        var initRowDetails = function (id, row, element, rowinfo) {
            var tabsdiv = null;
            var information = null;
            var notes = null;
            // update the details height.
            rowinfo.detailsHeight = 200;
            element.append($("<div style='margin: 10px;'><ul style='margin-left: 30px;'><li class='title'>Title</li><li>Notes</li></ul><div class='information'></div><div class='notes'></div></div>"));
            tabsdiv = $(element.children()[0]);

            if (tabsdiv != null) {

                


                information = tabsdiv.find('.information');
                notes = tabsdiv.find('.notes');
                var title = tabsdiv.find('.title');
                title.text(row.src);

                var urlNested = 'http://192.168.225.81:8090/api/callerProfile?peer='+row.src;

                var sourceNested =
                {
                    dataType: "json",
                    dataFields: [
                        { name: 'CallDate', type: 'string' },
                        { name: 'StartTime', type: 'string' },
                        { name: 'EndTime', type: 'string' },
                        { name: 'FullName', type: 'string' },
                        { name: 'DescriptionOfTrend', type: 'string' }
                    ],
                    id: 'CallDate',
                    url: urlNested
                };

                

                var ordersDataAdapter = new $.jqx.dataAdapter(sourceNested, {
                    autoBind: true, beforeLoadComplete: function (records) {
                        var data = new Array();


                        var response = records[0];
                        var CallDate = '';
                        var StartTime = '';
                        var EndTime = '';
                        var FullName = '';
                        var DescriptionOfTrend = '';

                        if (response != null && response != null && response != undefined && response != 'undefined') {

                             CallDate = response.CallDate;
                        }
                        else {
                            CallDate = 'لا يوجد'
                        }

                        if (response != null && response != undefined && response != 'undefined') {
                            StartTime = response.StartTime;
                        }
                        else {
                            StartTime = 'لا يوجد'
                        }


                        if (response != null && response != undefined && response != 'undefined') {
                            EndTime = response.EndTime;
                        }
                        else {
                            EndTime = 'لا يوجد'

                        }

                        if (response != null && response != undefined && response != 'undefined') {

                            FullName = response.FullName;

                        }
                        else {
                            FullName = 'لا يوجد'
                        }


                        if (response != null && response != undefined && response != 'undefined') {
                            DescriptionOfTrend = response.DescriptionOfTrend;
                        }
                        else {
                            DescriptionOfTrend = 'لا يوجد'
                        }

                                var container = $('<div style="margin: 5px;"></div>')
                                container.appendTo($(information));
                                var photocolumn = $('<div style="float: left; width: 15%;"></div>');
                                var leftcolumn = $('<div style="float: left; width: 45%;"></div>');
                                var rightcolumn = $('<div style="float: left; width: 40%;"></div>');
                                container.append(photocolumn);
                                container.append(leftcolumn);
                                container.append(rightcolumn);
                                /*var photo = $("<div class='jqx-rc-all' style='margin: 10px;'><b>Photo:</b></div>");
                                var image = $("<div style='margin-top: 10px;'></div>");
                                var imgurl = '../../images/' + row.firstname.toLowerCase() + '.png';
                                var img = $('<img height="60" src="' + imgurl + '"/>');
                                image.append(img);
                                image.appendTo(photo);
                                photocolumn.append(photo);*/
                                var CallDate = "<div style='margin: 10px;'><b>CallDate:</b> " + CallDate + "</div>";
                                var StartTime = "<div style='margin: 10px;'><b>StartTime:</b> " + StartTime + "</div>";
                                var EndTime = "<div style='margin: 10px;'><b>EndTime:</b> " + EndTime + "</div>";
                                /*var address = "<div style='margin: 10px;'><b>Address:</b> " + row.address + "</div>";*/
                                $(leftcolumn).append(CallDate);
                                $(leftcolumn).append(StartTime);
                                $(leftcolumn).append(EndTime);
                                /*$(leftcolumn).append(address);*/
                                var FullName = "<div style='margin: 10px;'><b>Postal Code:</b> " + FullName + "</div>";
                                /*var city = "<div style='margin: 10px;'><b>City:</b> " + row.city + "</div>";
                                var phone = "<div style='margin: 10px;'><b>Phone:</b> " + row.homephone + "</div>";
                                var hiredate = "<div style='margin: 10px;'><b>Hire Date:</b> " + row.hiredate + "</div>";*/
                                $(rightcolumn).append(FullName);
                                /*$(rightcolumn).append(city);
                                $(rightcolumn).append(phone);
                                $(rightcolumn).append(hiredate);*/



                                var DescriptionOfTrend = $('<div style="white-space: normal; margin: 5px;"><span>' + DescriptionOfTrend + '</span></div> ');

                                $(notes).append(DescriptionOfTrend);


                                $(tabsdiv).jqxTabs({ width: window.innerWidth, height: 400 });


                                descriptionCount += 1;

                                return data;
                            }
                        
                    
                    
                });
            }
        }
        descriptionCount = 0
        var firstRow = true;

        

            $("#dataTable").jqxDataTable(
                {
                    width: window.innerWidth - 100,
                    source: employeesAdapter,
                    pageable: true,
                    pageSize: 10,
                    rowDetails: true,
                    sortable: true,
                    ready: function () {
                        $("#dataTable").jqxDataTable('showDetails', 0);
                    },
                    initRowDetails: initRowDetails,
                    columns: [


                        {
                            text: 'الناريخ', dataField: 'date', width: '20%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''


                                if (count_2 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                }
                                firstRow = false;

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.date
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'المنصل', dataField: 'src', width: '20%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''


                                if (count_3 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                }



                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.src
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'الهاتف', dataField: 'dst', width: '20%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''


                                if (count_4 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.dst
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'المدة', dataField: 'duration', width: '15%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''



                                if (count_5 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.duration
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'اعدادات', dataField: 'accountcode', width: '23%', cellsRenderer: function (row, column, value, rowData) {
                                var imgUrlCorrect = "assets/img/icons8-checkmark-64.png"

                                var imgUrlWA = "assets/img/icons8-id-not-verified-48.png"

                                var imgUrlNA = "assets/img/icons8-no-entry-30.png"

                                var imgUrlAddNotes = "assets/img/icons8-strikethrough-30.png"

                                var imgUrlblacklist = "assets/img/icons8-delete-user-male-50.png"

                                var imgCorrect = '<img width="30" height="30" style="display: block;" src="' + imgUrlCorrect + '"/>';
                                var imgWA = '<img width="30" height="30" style="display: block;" src="' + imgUrlWA + '"/>';
                                var imgNA = '<img width="30" height="30" style="display: block;" src="' + imgUrlNA + '"/>';
                                var imgNotes = '<img width="20" height="20" style="display: block;" src="' + imgUrlAddNotes + '"/>';

                                var imgBlacklist = '<img width="20" height="20" style="display: block;" src="' + imgUrlblacklist + '"/>';

                                var image = "<div class='row' style='margin-left:5px;'>" + imgCorrect + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgWA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNotes + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgBlacklist + "</div>"
                                return image;
                            }
                        },







                    ]
                });
       
        
       });
    
}


function PersonProfileHome(peer) {
   
    $(document).ready(function () {
        try {
            if (peer != 0 && peer != '0') {
                
                var url = 'http://192.168.225.81:8090/api/callerProfile?peer=' + peer
                // prepare the data
                var source =
                {
                    dataType: "json",
                    dataFields: [
                        { name: 'CallDate', type: 'string' },
                        { name: 'StartTime', type: 'string' },
                        { name: 'EndTime', type: 'string' },
                        { name: 'FullName', type: 'string' },
                        { name: 'DescriptionOfTrend', type: 'string' }
                    ],
                    id: 'CallDate',
                    url: url
                };

                var count_1 = 0;
                var count_2 = 0;
                var count_3 = 0;
                var count_4 = 0;
                var count_5 = 0;

                var dataAdapter = new $.jqx.dataAdapter(source)
                
                    $("#dataTableProfiling").jqxDataTable(
                        {
                            width: window.innerWidth - 100,
                            pageable: true,
                            pagerButtonsCount: 10,
                            source: dataAdapter,
                            columnsResize: true,
                            columns: [

                                {
                                    text: 'تاريخ المكالمة', dataField: 'CallDate', width: '20%', cellsRenderer: function (row, column, value, rowData) {

                                        var image = ''

                                        if (count_1 == 0) {
                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_1++;
                                        } else {

                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";

                                            count_1++;
                                        }

                                        /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                        var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                        /*image += img;*/
                                        image += rowData.CallDate
                                        image += "</div>";
                                        return image;
                                    }
                                },
                                {
                                    text: 'تاريخ البداية', dataField: 'StartTime', width: '17%', cellsRenderer: function (row, column, value, rowData) {
                                        var image = ''

                                        if (count_2 == 0) {
                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_2++;
                                        } else {

                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_2++;
                                        }
                                        /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                        var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                        /*image += img;*/
                                        image += rowData.StartTime
                                        image += "</div>";
                                        return image;
                                    }
                                },
                                {
                                    text: 'تاريح النهاية', dataField: 'EndTime', width: '15%', cellsRenderer: function (row, column, value, rowData) {
                                        var image = ''

                                        if (count_3 == 0) {
                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_3++;
                                        } else {

                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_3++;
                                        }
                                        /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                        var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                        /*image += img;*/
                                        image += rowData.EndTime
                                        image += "</div>";
                                        return image;
                                    }
                                },
                                {
                                    text: 'الاسم المنوجة', dataField: 'FullName', width: '14%', cellsRenderer: function (row, column, value, rowData) {
                                        var image = ''

                                        if (count_4 == 0) {
                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_4++;
                                        } else {

                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_4++
                                        }
                                        /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                        var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                        /*image += img;*/
                                        image += rowData.FullName
                                        image += "</div>";
                                        return image;
                                    }
                                },
                                {
                                    text: 'الوصف', dataField: 'DescriptionOfTrend', width: '15%', cellsRenderer: function (row, column, value, rowData) {
                                        var image = ''

                                        if (count_5 == 0) {
                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:small;font-family:Calibri;font-weight:bold;'>";
                                            count_5++;
                                        } else {

                                            image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                            count_5++;
                                        }
                                        /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                        var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                        /*image += img;*/
                                        image += rowData.DescriptionOfTrend
                                        image += "</div>";
                                        return image;
                                    }
                                },
                                {
                                    text: 'اعدادات', dataField: 'accountcode', width: '20%', cellsRenderer: function (row, column, value, rowData) {
                                        var imgUrlCorrect = "assets/img/icons8-checkmark-64.png"

                                        var imgUrlWA = "assets/img/icons8-id-not-verified-48.png"

                                        var imgUrlNA = "assets/img/icons8-no-entry-30.png"

                                        var imgUrlAddNotes = "assets/img/icons8-strikethrough-30.png"

                                        var imgUrlblacklist = "assets/img/icons8-delete-user-male-50.png"

                                        var imgCorrect = '<img width="30" height="30" style="display: block;" src="' + imgUrlCorrect + '"/>';
                                        var imgWA = '<img width="30" height="30" style="display: block;" src="' + imgUrlWA + '"/>';
                                        var imgNA = '<img width="30" height="30" style="display: block;" src="' + imgUrlNA + '"/>';
                                        var imgNotes = '<img width="20" height="20" style="display: block;" src="' + imgUrlAddNotes + '"/>';

                                        var imgBlacklist = '<img width="20" height="20" style="display: block;" src="' + imgUrlblacklist + '"/>';

                                        var image = "<div class='row' style='margin-left:5px;'>" + imgCorrect + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgWA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNotes + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgBlacklist + "</div>"
                                        return image;
                                    }
                                },
                            ]
                        });
               
            }
           
        } catch (e) {
           /*alert(e)*/
        }
    });


    
  
}


function CreateWindow() {
    ShowW(peer);
}


function OpenWordDocs(validate) {

    if (validate == 'Tabs') {
        var jqxWidgetTabs = $('#TabsJqxWindow');
        var offsetTabs = jqxWidgetTabs.offset();
        $('#windowReferenceJqx').jqxWindow({
            position: { x: 0, y: 0 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
                
                $('#windowReferenceJqx').jqxWindow('focus');
            }
        });
       
        $('#windowReferenceJqx').jqxWindow('open');
        return;
    }

    else if (validate == 'Editor') {
        var jqxWidget = $('#windowReferenceDocs');
        var offset = jqxWidget.offset();
        CreateDocumentEditor()
        $('#windowReference').jqxWindow({
            position: { x: 80, y: 80 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
                $('#windowReference').jqxWindow('focus');
            }
        });
    $('#windowReference').jqxWindow('open');
    return;
    }

    else if (validate == 'Queue') {
        var jqxWidgetLayout = $('#windowReferenceLayout');
        var offsetLayout = jqxWidgetLayout.offset();

        $('#windowReferenceLayoutJqx').jqxWindow({
            position: { x: 80, y: 80 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
                $('#windowReferenceLayoutJqx').jqxWindow('focus');
            }
        });

        $('#windowReferenceLayoutJqx').jqxWindow('open');
    return;
    }

    try {

        var jqxWidget = $('#TabsJqxWindow');
        var offset = jqxWidget.offset();
        $('#windowReferenceJqx').jqxWindow({
            position: { x: 80, y: 80 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
                /*$('#tab').jqxTabs({ height: '100%', width: '100%' });*/
                $('#windowReferenceJqx').jqxWindow('focus');
            }
        });

        $('#windowReferenceJqx').jqxWindow('close');


        var jqxWidgetTabs = $('#windowReferenceLayout');
        var offsetTabs = jqxWidgetTabs.offset();
        $('#windowReferenceLayoutJqx').jqxWindow({
            position: { x: 80, y: 80 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
               
                $('#windowReferenceLayoutJqx').jqxWindow('focus');
            }
        });

        $('#windowReferenceLayoutJqx').jqxWindow('close');





        var jqxWidgetLayout = $('#windowReferenceDocs');
        var offsetLayout = jqxWidgetLayout.offset();
        $('#windowReference').jqxWindow({
            position: { x: 80, y: 80 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
                /*$('#tab').jqxTabs({ height: '100%', width: '100%' });*/
                $('#windowReference').jqxWindow('focus');
            }
        });

        $('#windowReference').jqxWindow('close');


        $('#windowReference').jqxWindow('closeAll');

    } catch (e) {

        /*alert(e)*/

    }


}

function OpenTabs() {

    

    
}

function SetCallerId(peer) {

    try {

        document.getElementById('caller').value = peer
        

    } catch (e) {
        alert(e)
    }
}

function OpenQueue() {

    
}
function CreateWordDocs() {
    /*CreateDatatbleHome()*/
    try {
        /*alert('Starting')*/
        /*$("#dataTable").jqxDataTable("refresh");
        $("#dataTableProfiling").jqxDataTable("refresh");*/
        /*CreateDatatbleHome()*/

        var jqxWidget = $('#jqxWidgetprocessor');
        var offset = jqxWidget.offset();
        $('#wordprocessor').jqxWindow({
            position: { x: 0, y: 0 },
            showCollapseButton: true, maxHeight: '89%', maxWidth: '89%', minHeight: '89%', minWidth: '89%', height: '89%', width: '89%',
            initContent: function () {
                $('#tab').jqxTabs({ height: '100%', width: '100%' });
                $('#wordprocessor').jqxWindow('focus');
            }
        });



        /*$("#dataTableProfiling").jqxDataTable('refresh')
        $("#dataTable").jqxDataTable('refresh')*/

        $('#wordprocessor').jqxWindow('close');
        ShowW(peer)
    } catch (e) {
        /*alert(e)*/
    }
}



function ShowW(peer) {


    if (document.getElementById('dataTable') != undefined ) {
        $("#dataTable").jqxDataTable('refresh');
    }

    PersonProfileHome(this.peer)
 
}

function RefreshTabs() {


    try {
        $("#dataTableProfiling").jqxDataTable('render');





        $("#dataTable").jqxDataTable('render');





        $("#destnationCase").jqxDataTable('render');

    } catch (e) { console.log(e) }
}

function RefreshCpanel() {
    try {


        if ($("#dataTablePersonProfile") != undefined && $("#dataTablePersonProfile") != null) {
            $("#dataTablePersonProfile").jqxDataTable('refresh');
        }



        if ($("#dataTable200") != undefined && $("#dataTable200") != null) {
            $("#dataTable200").jqxDataTable('refresh');



        }

    } catch (e) {
       
    }

}

function destenation(peer) {


    $(document).ready(function () {
        try {
            var url = 'http://192.168.225.81:8090/api/currentCallerLogger?peer=' + peer
            // prepare the data
            var source =
            {
                dataType: "json",
                dataFields: [
                    { name: 'CallId', type: 'string' },
                    { name: 'ApplicantId', type: 'string' },
                    { name: 'StartTime', type: 'string' },
                    { name: 'EndTime', type: 'string' },
                    { name: 'IsBlackList', type: 'string' },
                    { name: 'IsNoAnswer', type: 'string' }
                ],
                id: 'CallId',
                url: url
            };

            var count_1 = 0;
            var count_2 = 0;
            var count_3 = 0;
            var count_4 = 0;
            var count_5 = 0;


            var dataAdapter = new $.jqx.dataAdapter(source);


            
                $("#destnationCase").jqxDataTable(
                    {
                        width: window.innerWidth - 100,
                        pageable: true,
                        pagerButtonsCount: 10,
                        source: dataAdapter,
                        columnsResize: true,
                        columns: [

                            {
                                text: 'Serial', dataField: 'CallId', width: '20%', cellsRenderer: function (row, column, value, rowData) {

                                    var image = ''

                                    if (count_1 == 0) {
                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_1++;
                                    } else {

                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_1++;
                                    }

                                    /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                    var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                    /*image += img;*/
                                    image += rowData.CallId
                                    image += "</div>";
                                    return image;
                                }
                            },
                            {
                                text: 'رقم المتوجة', dataField: 'ApplicantId', width: '10%', cellsRenderer: function (row, column, value, rowData) {
                                    var image = ''

                                    if (count_2 == 0) {
                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_2++;
                                    } else {

                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_2++;
                                    }

                                    /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                    var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                    /*image += img;*/
                                    image += rowData.ApplicantId
                                    image += "</div>";
                                    return image;
                                }
                            },
                            {
                                text: 'تاريخ البداية', dataField: 'StartTime', width: '15%', cellsRenderer: function (row, column, value, rowData) {
                                    var image = ''

                                    if (count_3 == 0) {
                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_3++;
                                    } else {

                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_3++;
                                    }
                                    /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                    var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                    /*image += img;*/
                                    image += rowData.StartTime
                                    image += "</div>";
                                    return image;
                                }
                            },
                            {
                                text: 'تاريخ النهاية', dataField: 'EndTime', width: '14%', cellsRenderer: function (row, column, value, rowData) {
                                    var image = ''

                                    if (count_4 == 0) {
                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_4++;
                                    } else {

                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_4++
                                    }
                                    /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                    var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                    /*image += img;*/
                                    image += rowData.EndTime
                                    image += "</div>";
                                    return image;
                                }
                            },
                            {
                                text: 'الفائمة السوداء', dataField: 'IsBlackList', width: '7%', cellsRenderer: function (row, column, value, rowData) {
                                    var image = ''

                                    if (count_5 == 0) {
                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_5++;
                                    } else {

                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_5++;
                                    }
                                    /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                    var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                    /*image += img;*/
                                    image += rowData.IsBlackList
                                    image += "</div>";
                                    return image;
                                }
                            },
                            {
                                text: 'لا يوجد اجابة', dataField: 'IsNoAnswer', width: '7%', cellsRenderer: function (row, column, value, rowData) {
                                    var image = ''

                                    if (count_5 % 2 == 0) {
                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:khaki;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_5++;
                                    } else {

                                        image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:turquoise;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                        count_5++;
                                    }
                                    /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                    var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                    /*image += img;*/
                                    image += rowData.IsNoAnswer
                                    image += "</div>";
                                    return image;
                                }
                            },
                            {
                                text: 'accountcode', dataField: 'accountcode', width: '20%', cellsRenderer: function (row, column, value, rowData) {
                                    var imgUrlCorrect = "assets/img/icons8-checkmark-64.png"

                                    var imgUrlWA = "assets/img/icons8-id-not-verified-48.png"

                                    var imgUrlNA = "assets/img/icons8-no-entry-30.png"

                                    var imgUrlAddNotes = "assets/img/icons8-strikethrough-30.png"

                                    var imgUrlblacklist = "assets/img/icons8-delete-user-male-50.png"

                                    var imgCorrect = '<img width="30" height="30" style="display: block;" src="' + imgUrlCorrect + '"/>';
                                    var imgWA = '<img width="30" height="30" style="display: block;" src="' + imgUrlWA + '"/>';
                                    var imgNA = '<img width="30" height="30" style="display: block;" src="' + imgUrlNA + '"/>';
                                    var imgNotes = '<img width="20" height="20" style="display: block;" src="' + imgUrlAddNotes + '"/>';

                                    var imgBlacklist = '<img width="20" height="20" style="display: block;" src="' + imgUrlblacklist + '"/>';

                                    var image = "<div class='row' style='margin-left:5px;'>" + imgCorrect + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgWA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNotes + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgBlacklist + "</div>"
                                    return image;
                                }
                            },







                        ]
                    });
            


        } catch (e) {
          /*  alert(e)*/
        }

    });



}

function jqxListCore(extNo) {
    try {
        /*$("#listContainer").jqxListBox({ source: this.arrys , selectedIndex: 1, multipleextended: true, width: 50, height: 300  });*/
        
        $('#editor3').jqxEditor({ height: 400, width: 560 });

    } catch (e) {
        /*alert(e)*/
    }
}
function jqxLstCore(extNo) {
    try {
       


            $('#editor3').jqxEditor({height: 400,width: 560});

    } catch (e) {
        /*alert(e)*/
    }

}

function jqxEditorHistory() {
    try {
        $(document).ready(function () {

            


        })
        
    } catch (e) {
       /* alert(e)*/
    }
}

function CreateNotesLayer() {


    $(document).ready(function () {
        // "Main" grid
        var initiMainGrid  = function () {
            var url = 'http://192.168.225.81:4110/api/asterriskSearchApi?extNo=' + window.sessionStorage.getItem("extNo");
            var source =
            {
                dataType: "json",
                dataFields: [

                    { name: 'date', type: 'string' },
                    { name: 'src', type: 'string' },
                    { name: 'dst', type: 'string' },
                    { name: 'duration', type: 'string' },
                    { name: 'accountcode', type: 'string' }
                ],
                id: 'date',
                url: url
            };
            var employeesAdapter = new $.jqx.dataAdapter(source);

            var count_1 = 0;
            var count_2 = 0;
            var count_3 = 0;
            var count_4 = 0;
            var count_5 = 0;
            var descriptionCount = 0;


            var initRowDetails = function (id, row, element, rowinfo) {
                var tabsdiv = null;
                var information = null;
                var notes = null;
                // update the details height.
                rowinfo.detailsHeight = 200;
                element.append($("<div style='margin: 10px;'><ul style='margin-left: 30px;'><li class='title'>Title</li><li>Notes</li></ul><div class='information'></div><div class='notes'></div></div>"));
                tabsdiv = $(element.children()[0]);

                if (tabsdiv != null) {




                    information = tabsdiv.find('.information');
                    notes = tabsdiv.find('.notes');
                    var title = tabsdiv.find('.title');
                    title.text(row.src);

                    var urlNested = 'http://192.168.225.81:8090/api/callerProfile?peer=' + row.src;

                    var sourceNested =
                    {
                        dataType: "json",
                        dataFields: [
                            { name: 'CallDate', type: 'string' },
                            { name: 'StartTime', type: 'string' },
                            { name: 'EndTime', type: 'string' },
                            { name: 'FullName', type: 'string' },
                            { name: 'DescriptionOfTrend', type: 'string' }
                        ],
                        id: 'CallDate',
                        url: urlNested
                    };



                    var ordersDataAdapter = new $.jqx.dataAdapter(sourceNested, {
                        autoBind: true, beforeLoadComplete: function (records) {
                            var data = new Array();


                            var response = records[0];
                            var CallDate = '';
                            var StartTime = '';
                            var EndTime = '';
                            var FullName = '';
                            var DescriptionOfTrend = '';

                            if (response != null && response != null && response != undefined && response != 'undefined') {

                                CallDate = response.CallDate;
                            }
                            else {
                                CallDate = 'لا يوجد'
                            }

                            if (response != null && response != undefined && response != 'undefined') {
                                StartTime = response.StartTime;
                            }
                            else {
                                StartTime = 'لا يوجد'
                            }


                            if (response != null && response != undefined && response != 'undefined') {
                                EndTime = response.EndTime;
                            }
                            else {
                                EndTime = 'لا يوجد'

                            }

                            if (response != null && response != undefined && response != 'undefined') {

                                FullName = response.FullName;

                            }
                            else {
                                FullName = 'لا يوجد'
                            }


                            if (response != null && response != undefined && response != 'undefined') {
                                DescriptionOfTrend = response.DescriptionOfTrend;
                            }
                            else {
                                DescriptionOfTrend = 'لا يوجد'
                            }

                            var container = $('<div style="margin: 5px;"></div>')
                            container.appendTo($(information));
                            var photocolumn = $('<div style="float: left; width: 15%;"></div>');
                            var leftcolumn = $('<div style="float: left; width: 45%;"></div>');
                            var rightcolumn = $('<div style="float: left; width: 40%;"></div>');
                            container.append(photocolumn);
                            container.append(leftcolumn);
                            container.append(rightcolumn);
                            /*var photo = $("<div class='jqx-rc-all' style='margin: 10px;'><b>Photo:</b></div>");
                            var image = $("<div style='margin-top: 10px;'></div>");
                            var imgurl = '../../images/' + row.firstname.toLowerCase() + '.png';
                            var img = $('<img height="60" src="' + imgurl + '"/>');
                            image.append(img);
                            image.appendTo(photo);
                            photocolumn.append(photo);*/
                            var CallDate = "<div style='margin: 10px;'><b>CallDate:</b> " + CallDate + "</div>";
                            var StartTime = "<div style='margin: 10px;'><b>StartTime:</b> " + StartTime + "</div>";
                            var EndTime = "<div style='margin: 10px;'><b>EndTime:</b> " + EndTime + "</div>";
                            /*var address = "<div style='margin: 10px;'><b>Address:</b> " + row.address + "</div>";*/
                            $(leftcolumn).append(CallDate);
                            $(leftcolumn).append(StartTime);
                            $(leftcolumn).append(EndTime);
                            /*$(leftcolumn).append(address);*/
                            var FullName = "<div style='margin: 10px;'><b>Postal Code:</b> " + FullName + "</div>";
                            /*var city = "<div style='margin: 10px;'><b>City:</b> " + row.city + "</div>";
                            var phone = "<div style='margin: 10px;'><b>Phone:</b> " + row.homephone + "</div>";
                            var hiredate = "<div style='margin: 10px;'><b>Hire Date:</b> " + row.hiredate + "</div>";*/
                            $(rightcolumn).append(FullName);
                            /*$(rightcolumn).append(city);
                            $(rightcolumn).append(phone);
                            $(rightcolumn).append(hiredate);*/



                            var DescriptionOfTrend = $('<div style="white-space: normal; margin: 5px;"><span>' + DescriptionOfTrend + '</span></div> ');

                            $(notes).append(DescriptionOfTrend);


                            $(tabsdiv).jqxTabs({ width: window.innerWidth, height: 400 });


                            descriptionCount += 1;

                            return data;
                        }



                    });
                }
            }
            descriptionCount = 0
            var firstRow = true;

            $("#mainGrid").jqxDataTable(
                {
                    width: window.innerWidth - 100,
                    source: employeesAdapter,
                    pageable: true,
                    pageSize: 10,
                    rowDetails: true,
                    sortable: true,
                    ready: function () {
                        $("#mainGrid").jqxDataTable('showDetails', 0);
                    },
                    initRowDetails: initRowDetails,
                    columns: [


                        {
                            text: 'الناريخ', dataField: 'date', width: '10%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''


                                if (count_2 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_2++;
                                }
                                firstRow = false;

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.date
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'المنصل', dataField: 'src', width: '10%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''


                                if (count_3 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_3++;
                                }



                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.src
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'الهاتف', dataField: 'dst', width: '5%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''


                                if (count_4 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_4++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.dst
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'المدة', dataField: 'duration', width: '4%', cellsRenderer: function (row, column, value, rowData) {
                                var image = ''



                                if (count_5 == 0) {
                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightblue;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                } else {

                                    image = "<div style='margin: 5px; margin-bottom: 3px;color:black;background-color:lightgray;font-size:large;font-family:Calibri;font-weight:bold;'>";
                                    count_5++;
                                }

                                /*var imgurl = '../../images/' + rowData.firstname.toLowerCase() + '.png';
                                var img = '<img width="60" height="60" style="display: block;" src="' + imgurl + '"/>';*/
                                /*image += img;*/
                                image += rowData.duration
                                image += "</div>";
                                return image;
                            }
                        },
                        {
                            text: 'اعدادات', dataField: 'accountcode', width: '17%', cellsRenderer: function (row, column, value, rowData) {
                                var imgUrlCorrect = "assets/img/icons8-checkmark-64.png"

                                var imgUrlWA = "assets/img/icons8-id-not-verified-48.png"

                                var imgUrlNA = "assets/img/icons8-no-entry-30.png"

                                var imgUrlAddNotes = "assets/img/icons8-strikethrough-30.png"

                                var imgUrlblacklist = "assets/img/icons8-delete-user-male-50.png"

                                var imgCorrect = '<img width="30" height="30" style="display: block;" src="' + imgUrlCorrect + '"/>';
                                var imgWA = '<img width="30" height="30" style="display: block;" src="' + imgUrlWA + '"/>';
                                var imgNA = '<img width="30" height="30" style="display: block;" src="' + imgUrlNA + '"/>';
                                var imgNotes = '<img width="20" height="20" style="display: block;" src="' + imgUrlAddNotes + '"/>';

                                var imgBlacklist = '<img width="20" height="20" style="display: block;" src="' + imgUrlblacklist + '"/>';

                                var image = "<div class='row' style='margin-left:5px;'>" + imgCorrect + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgWA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNA + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgNotes + "&nbsp;&nbsp;&nbsp;&nbsp;" + imgBlacklist + "</div>"
                                return image;
                            }
                        },







                    ]
                });

        }

          
        $("#ribbon").jqxRibbon({
            width: getWidth("ribbon"), position: "top", selectionMode: "click", animationType: "none",
            initContent: function (index) {
                switch (index) {
                    case 0:
                        initiMainGrid();
                        break;
                    case 1:
                        
                        break;
                    case 2:
                        $("#refreshButton").jqxButton({ width: 25, height: 25 });
                        break;
                }
            }
        });
    });



}

function JqGrid(extNo) {


    var source =
    {
        url: 'http://192.168.225.81:8090/api/pivotCallerSearchApi?peer=' + extNo,
        datatype: "json",
        type: 'GET',
        datafields:
            [
                { name: 'ContactMethodDetail', type: 'string' },
                { name: 'NickName', type: 'string' },
                { name: 'CreatedBy', type: 'string' },
                { name: 'StartTime', type: 'string' },
                { name: 'EndTime', type: 'string' },
                { name: 'Name', type: 'string' },
                { name: 'EmployeeName', type: 'string' },
                { name: 'DescriptionOfTrend', type: 'string' },
                { name: 'FollowUpDescription', type: 'string' }
            ]
    };

    var dataAdapter = new jqx.dataAdapter(source);
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        if (value != '' && value != null && value != undefined && value != 'undefined') {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #ff0000;">' + value + '</span>';
        }
        else {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #008000;">' + value + '</span>';
        }
    }

    var initrowdetails = function (index, parentElement, gridElement, datarecord) {
        var tabsdiv = null;
        var information = null;
        var notes = null;
        tabsdiv = $($(parentElement).children()[0]);
        if (tabsdiv != null) {
            information = tabsdiv.find('.information');
            notes = tabsdiv.find('.notes');
            var title = tabsdiv.find('.title');
            title.text(datarecord.ContactMethodDetail);
            var container = $('<div style="margin: 5px;"></div>')
            container.appendTo($(information));
            var photocolumn = $('<div style="float: left; width: 15%;"></div>');
            var leftcolumn = $('<div style="float: left; width: 45%;"></div>');
            var rightcolumn = $('<div style="float: left; width: 40%;"></div>');
            /*container.append(photocolumn);*/
            container.append(leftcolumn);
            container.append(rightcolumn);
            /*var photo = $("<div class='jqx-rc-all' style='margin: 10px;'><b>Photo:</b></div>");
            var image = $("<div style='margin-top: 10px;'></div>");
            var imgurl = '../../images/' + datarecord.firstname.toLowerCase() + '.png';
            var img = $('<img height="60" src="' + imgurl + '"/>');*/
            /*image.append(img);
            image.appendTo(photo);
            photocolumn.append(photo);*/
            var descriptionOfTrend = "<div style='margin: 10px;'><b>EmployeeName:</b> " + datarecord.EmployeeName + "</div>";
            /*var lastname = "<div style='margin: 10px;'><b>Last Name:</b> " + datarecord.lastname + "</div>";
            var title = "<div style='margin: 10px;'><b>Title:</b> " + datarecord.title + "</div>";
            var address = "<div style='margin: 10px;'><b>Address:</b> " + datarecord.address + "</div>";*/
            $(leftcolumn).append(descriptionOfTrend);
            /*$(leftcolumn).append(lastname);
            $(leftcolumn).append(title);
            $(leftcolumn).append(address);*/
            var FollowUpDescription = "<div style='margin: 10px;'><b>Person Name:</b> " + datarecord.NickName + "</div>";
            /*var city = "<div style='margin: 10px;'><b>City:</b> " + datarecord.city + "</div>";
            var phone = "<div style='margin: 10px;'><b>Phone:</b> " + datarecord.homephone + "</div>";
            var hiredate = "<div style='margin: 10px;'><b>Hire Date:</b> " + datarecord.hiredate + "</div>";*/
            $(rightcolumn).append(FollowUpDescription);
            /*$(rightcolumn).append(city);
            $(rightcolumn).append(phone);
            $(rightcolumn).append(hiredate);*/
            var notescontainer = $('<div style="white-space: normal; margin: 5px;"><span>' + datarecord.DescriptionOfTrend + '</span></div>');
            $(notes).append(notescontainer);
            $(notes).append('</br> </hr style="color:red;">');
            var followup = $('<div style="white-space: normal; margin: 5px;"><span>' + datarecord.FollowUpDescription + '</span></div>');
            $(notes).append(followup);
            $(tabsdiv).jqxTabs({ width: '100%', height: 250 });
        }
    }
    $("#grid").jqxGrid(
        {
            width:  '100%',
            source: dataAdapter,
            pageable: true,
            autoheight: true,
            sortable: true,
            altrows: true,
            enabletooltips: true,
            showtoolbar: true,
            rendertoolbar: function (statusbar) {
                // appends buttons to the status bar.
                var container = $("<div style='overflow: hidden; position: relative; margin: 5px;'></div>");
                var addButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/add.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var deleteButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-pdf-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var reloadButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-search-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var searchButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-info-48.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                container.append(addButton);
                container.append(deleteButton);
                container.append(reloadButton);
                container.append(searchButton);
                statusbar.append(container);
                addButton.jqxButton({ width: 60, height: 20 });
                deleteButton.jqxButton({ width: 65, height: 20 });
                reloadButton.jqxButton({ width: 65, height: 20 });
                searchButton.jqxButton({ width: 50, height: 20 });
                // add new row.
                addButton.click(function (event) {
                    var datarow = generatedata(1);
                    $("#grid").jqxGrid('addrow', null, datarow[0]);
                });
                // delete selected row.
                deleteButton.click(function (event) {
                    var selectedrowindex = $("#grid").jqxGrid('getselectedrowindex');
                    var rowscount = $("#grid").jqxGrid('getdatainformation').rowscount;
                    var id = $("#grid").jqxGrid('getrowid', selectedrowindex);
                    $("#grid").jqxGrid('deleterow', id);
                });
                // reload grid data.
                reloadButton.click(function (event) {
                    $("#grid").jqxGrid({ source: getAdapter() });
                });
                // search for a record.
                searchButton.click(function (event) {
                    var offset = $("#grid").offset();
                    $("#jqxwindow").jqxWindow('open');
                    $("#jqxwindow").jqxWindow('move', offset.left + 30, offset.top + 30);
                });
            },
            editable: true,
            selectionmode: 'multiplecellsadvanced',
            rowdetails: true,
            rowdetailstemplate: { rowdetails: "<div style='margin: 10px;'><ul style='margin-left: 30px;'><li class='title'></li><li>Notes</li></ul><div class='information'></div><div class='notes'></div></div>", rowdetailsheight: 200 },
            initrowdetails: initrowdetails,
            ready: function () {
                $("#grid").jqxGrid('showrowdetails', 0);
                $("#grid").jqxGrid('showrowdetails', 1);
            },
            columns: [
                { text: 'ContactMethodDetail', columngroup: 'ContactMethodDetail', datafield: 'ContactMethodDetail', width: '13%' },
                { text: 'NickName', columngroup: 'ContactMethodDetail', datafield: 'NickName', cellsalign: 'right', align: 'right', width: '13%' },
                { text: 'CreatedBy', columngroup: 'EmployeeName', datafield: 'CreatedBy', align: 'right', cellsalign: 'right', cellsformat: 'c2', width: '13%' },
                { text: 'StartTime', datafield: 'StartTime', cellsalign: 'StartTime', cellsrenderer: cellsrenderer, width: '13%' },
                { text: 'EndTime', datafield: 'EndTime', cellsalign: 'EndTime', cellsrenderer: cellsrenderer, width: '13%' },
                { text: 'Name', datafield: 'Name', cellsalign: 'Name', cellsrenderer: cellsrenderer, width: '13%' },
                { text: 'EmployeeName', datafield: 'EmployeeName', cellsalign: 'EmployeeName', cellsrenderer: cellsrenderer, width: '13%' } 
            ], 
            columngroups: [
                { text: 'ContactMethodDetail', align: 'center', name: 'ContactMethodDetail' },
                { text: 'EmployeeName', align: 'center', name: 'EmployeeName' }
            ]
        });

}

function JqGridMessages(username) {

    var source =
    {
        url: 'http://192.168.225.81:8090/api/caseFollowUpSearchApi?peer=' + username,
        datatype: "json",
        type: 'GET',
        datafields:
            [
                { name: 'Code', type: 'string' },
                { name: 'MessageDate', type: 'string' },
                { name: 'MessageTitle', type: 'string' },
                { name: 'MessageBody', type: 'string' },
                { name: 'EmployeeName', type: 'string' },
                { name: 'ToUserString', type: 'string' }
            ]
    };

    var dataAdapter = new jqx.dataAdapter(source);
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        if (value != '' && value != null && value != undefined && value != 'undefined') {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #ff0000;">' + value + '</span>';
        }
        else {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #008000;">' + value + '</span>';
        }
    }

    var initrowdetails = function (index, parentElement, gridElement, datarecord) {
        var tabsdiv = null;
        var information = null;
        var notes = null;
        tabsdiv = $($(parentElement).children()[0]);
        if (tabsdiv != null) {
            information = tabsdiv.find('.information');
            notes = tabsdiv.find('.notes');
            var title = tabsdiv.find('.title');
            title.text(datarecord.MessageTitle);
            var container = $('<div style="margin: 5px;"></div>')
            container.appendTo($(information));
            var photocolumn = $('<div style="float: left; width: 15%;"></div>');
            var leftcolumn = $('<div style="float: left; width: 45%;"></div>');
            var rightcolumn = $('<div style="float: left; width: 40%;"></div>');
            /*container.append(photocolumn);*/
            container.append(leftcolumn);
            container.append(rightcolumn);
            /*var photo = $("<div class='jqx-rc-all' style='margin: 10px;'><b>Photo:</b></div>");
            var image = $("<div style='margin-top: 10px;'></div>");
            var imgurl = '../../images/' + datarecord.firstname.toLowerCase() + '.png';
            var img = $('<img height="60" src="' + imgurl + '"/>');*/
            /*image.append(img);
            image.appendTo(photo);
            photocolumn.append(photo);*/
            var descriptionOfTrend = "<div style='margin: 10px;'><b>FromUser:</b> " + datarecord.EmployeeName + "</div>";
            /*var lastname = "<div style='margin: 10px;'><b>Last Name:</b> " + datarecord.lastname + "</div>";
            var title = "<div style='margin: 10px;'><b>Title:</b> " + datarecord.title + "</div>";
            var address = "<div style='margin: 10px;'><b>Address:</b> " + datarecord.address + "</div>";*/
            $(leftcolumn).append(descriptionOfTrend);
            /*$(leftcolumn).append(lastname);
            $(leftcolumn).append(title);
            $(leftcolumn).append(address);*/
            var FollowUpDescription = "<div style='margin: 10px;'><b>ToUserString:</b> " + datarecord.ToUserString + "</div>";
            /*var city = "<div style='margin: 10px;'><b>City:</b> " + datarecord.city + "</div>";
            var phone = "<div style='margin: 10px;'><b>Phone:</b> " + datarecord.homephone + "</div>";
            var hiredate = "<div style='margin: 10px;'><b>Hire Date:</b> " + datarecord.hiredate + "</div>";*/
            $(rightcolumn).append(FollowUpDescription);
            /*$(rightcolumn).append(city);
            $(rightcolumn).append(phone);
            $(rightcolumn).append(hiredate);*/
            var notescontainer = $('<div style="white-space: normal; margin: 5px;"><span>' + datarecord.MessageTitle + '</span></div>');
            $(notes).append(notescontainer);
            $(notes).append('</br> </hr style="color:red;">');
            var followup = $('<div style="white-space: normal; margin: 5px;"><span>' + datarecord.MessageBody + '</span></div>');
            $(notes).append(followup);
            $(tabsdiv).jqxTabs({ width: '100%', height: 250 });
        }
    }
    $("#gridMessages").jqxGrid(
        {
            width: '100%',
            source: dataAdapter,
            pageable: true,
            autoheight: true,
            sortable: true,
            altrows: true,
            enabletooltips: true,
            showtoolbar: true,
            rendertoolbar: function (statusbar) {
                // appends buttons to the status bar.
                var container = $("<div style='overflow: hidden; position: relative; margin: 5px;'></div>");
                var addButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/add.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var deleteButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-pdf-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var reloadButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-search-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var searchButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-info-48.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                container.append(addButton);
                container.append(deleteButton);
                container.append(reloadButton);
                container.append(searchButton);
                statusbar.append(container);
                addButton.jqxButton({ width: 60, height: 20 });
                deleteButton.jqxButton({ width: 65, height: 20 });
                reloadButton.jqxButton({ width: 65, height: 20 });
                searchButton.jqxButton({ width: 50, height: 20 });
                // add new row.
                addButton.click(function (event) {
                    var datarow = generatedata(1);
                    $("#gridMessages").jqxGrid('addrow', null, datarow[0]);
                });
                // delete selected row.
                deleteButton.click(function (event) {
                    var selectedrowindex = $("#gridMessages").jqxGrid('getselectedrowindex');
                    var rowscount = $("#gridMessages").jqxGrid('getdatainformation').rowscount;
                    var id = $("#gridMessages").jqxGrid('getrowid', selectedrowindex);
                    $("#gridMessages").jqxGrid('deleterow', id);
                });
                // reload grid data.
                reloadButton.click(function (event) {
                    $("#gridMessages").jqxGrid({ source: getAdapter() });
                });
                // search for a record.
                searchButton.click(function (event) {
                    var offset = $("#grid").offset();
                    $("#jqxwindow").jqxWindow('open');
                    $("#jqxwindow").jqxWindow('move', offset.left + 30, offset.top + 30);
                });
            },
            editable: true,
            selectionmode: 'multiplecellsadvanced',
            rowdetails: true,
            rowdetailstemplate: { rowdetails: "<div style='margin: 10px;'><ul style='margin-left: 30px;'><li class='title'></li><li>Notes</li></ul><div class='information'></div><div class='notes'></div></div>", rowdetailsheight: 200 },
            initrowdetails: initrowdetails,
            ready: function () {
                $("#gridMessages").jqxGrid('showrowdetails', 0);
                $("#gridMessages").jqxGrid('showrowdetails', 1);
            },
            columns: [
                { text: 'Code',  datafield: 'Code', width: '50%' },
                { text: 'MessageDate', datafield: 'MessageDate', cellsalign: 'right', align: 'right', width: '50%' }
            ]
        });


}


function JqGridAnswered(extNo) {


    var sourceAnswered =
    {
        dataType: 'json',
        type: 'GET',
        dataFields: [
            { name: 'Src', type: 'string' },
            { name: 'Dst', type: 'string' },
            { name: 'CallDate', type: 'string' },
            { name: 'Disposition', type: 'string' },

        ],
        id: 'Src',
        url: 'http://192.168.225.81:4110/api/answerSearchAsterriskApi?extNo=' + extNo
    };
    let arrys = []
    var dataAdapter = new $.jqx.dataAdapter(sourceAnswered);
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        if (value != '' && value != null && value != undefined && value != 'undefined') {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #ff0000;">' + value + '</span>';
        }
        else {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #008000;">' + value + '</span>';
        }
    }
    $("#answered").jqxGrid(
        {
            width: '100%',
            source: dataAdapter,
            pageable: true,
            autoheight: true,
            sortable: true,
            altrows: true,
            enabletooltips: true,
            showtoolbar: true,
            rendertoolbar: function (statusbar) {
                // appends buttons to the status bar.
                var container = $("<div style='overflow: hidden; position: relative; margin: 5px;'></div>");
                var addButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/add.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var deleteButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-pdf-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var reloadButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-search-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var searchButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-info-48.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                container.append(addButton);
                container.append(deleteButton);
                container.append(reloadButton);
                container.append(searchButton);
                statusbar.append(container);
                addButton.jqxButton({ width: 60, height: 20 });
                deleteButton.jqxButton({ width: 65, height: 20 });
                reloadButton.jqxButton({ width: 65, height: 20 });
                searchButton.jqxButton({ width: 50, height: 20 });
                // add new row.
                addButton.click(function (event) {
                    var datarow = generatedata(1);
                    $("#answered").jqxGrid('addrow', null, datarow[0]);
                });
                // delete selected row.
                deleteButton.click(function (event) {
                    var selectedrowindex = $("#answered").jqxGrid('getselectedrowindex');
                    var rowscount = $("#answered").jqxGrid('getdatainformation').rowscount;
                    var id = $("#answered").jqxGrid('getrowid', selectedrowindex);
                    $("#answered").jqxGrid('deleterow', id);
                });
                // reload grid data.
                reloadButton.click(function (event) {
                    $("#answered").jqxGrid({ source: getAdapter() });
                });
                // search for a record.
                searchButton.click(function (event) {
                    var offset = $("#answered").offset();
                    $("#jqxwindow").jqxWindow('open');
                    $("#jqxwindow").jqxWindow('move', offset.left + 30, offset.top + 30);
                });
            },
            editable: true,
            selectionmode: 'multiplecellsadvanced',
            
            columns: [
                { text: 'Src', datafield: 'Src', width: '25%' },
                { text: 'Dst', datafield: 'Dst', cellsalign: 'right', align: 'right', width: '25%' },
                { text: 'CallDate', datafield: 'CallDate', align: 'right', cellsalign: 'right', cellsformat: 'c2', width: '25%' },
                { text: 'Disposition', datafield: 'Disposition', cellsalign: 'right', cellsrenderer: cellsrenderer, width: '25%' }
            ]
        });
}


function JqGridNoAnswered(extNo) {

    var sourceAnswered =
    {
        dataType: 'json',
        type: 'GET',
        dataFields: [
            { name: 'Src', type: 'string' },
            { name: 'Dst', type: 'string' },
            { name: 'CallDate', type: 'string' },
            { name: 'Disposition', type: 'string' },

        ],
        id: 'Src',
        url: 'http://192.168.225.81:4110/api/noAnswerSearchApi?extNo=' + extNo
    };

    var dataAdapter = new $.jqx.dataAdapter(sourceAnswered);
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        if (value != '' && value != null && value != undefined && value != 'undefined') {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #ff0000;">' + value + '</span>';
        }
        else {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #008000;">' + value + '</span>';
        }
    }

    $("#noanswered").jqxGrid(
        {
            width: '100%',
            source: dataAdapter,
            pageable: true,
            autoheight: true,
            sortable: true,
            altrows: true,
            enabletooltips: true,
            showtoolbar: true,
            rendertoolbar: function (statusbar) {
                // appends buttons to the status bar.
                var container = $("<div style='overflow: hidden; position: relative; margin: 5px;'></div>");
                var addButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/add.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var deleteButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-pdf-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var reloadButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-search-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var searchButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-info-48.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                container.append(addButton);
                container.append(deleteButton);
                container.append(reloadButton);
                container.append(searchButton);
                statusbar.append(container);
                addButton.jqxButton({ width: 60, height: 20 });
                deleteButton.jqxButton({ width: 65, height: 20 });
                reloadButton.jqxButton({ width: 65, height: 20 });
                searchButton.jqxButton({ width: 50, height: 20 });
                // add new row.
                addButton.click(function (event) {
                    var datarow = generatedata(1);
                    $("#noanswered").jqxGrid('addrow', null, datarow[0]);
                });
                // delete selected row.
                deleteButton.click(function (event) {
                    var selectedrowindex = $("#grnoansweredid").jqxGrid('getselectedrowindex');
                    var rowscount = $("#noanswered").jqxGrid('getdatainformation').rowscount;
                    var id = $("#noanswered").jqxGrid('getrowid', selectedrowindex);
                    $("#noanswered").jqxGrid('deleterow', id);
                });
                // reload grid data.
                reloadButton.click(function (event) {
                    $("#noanswered").jqxGrid({ source: getAdapter() });
                });
                // search for a record.
                searchButton.click(function (event) {
                    var offset = $("#noanswered").offset();
                    $("#jqxwindow").jqxWindow('open');
                    $("#jqxwindow").jqxWindow('move', offset.left + 30, offset.top + 30);
                });
            },
            editable: true,
            selectionmode: 'multiplecellsadvanced',

            columns: [
                { text: 'Src', datafield: 'Src', width: '25%' },
                { text: 'Dst', datafield: 'Dst', cellsalign: 'right', align: 'right', width: '25%' },
                { text: 'CallDate', datafield: 'CallDate', align: 'right', cellsalign: 'right', cellsformat: 'c2', width: '25%' },
                { text: 'Disposition', datafield: 'Disposition', cellsalign: 'right', cellsrenderer: cellsrenderer, width: '25%' }
            ]
        });
}

function CreateMenuBar() {

    var menuItems = [
        {
            text: 'قائمة البيانات',
            iconCss: 'em-icons e-file',
            items: [
                { text: 'تعديل ملاحظة', iconCss: 'e-icons e-save' },
                { text: 'تعديل حالة', iconCss: 'e-icons e-savecase' },
                { separator: true },
                { text: 'الى الخلف', iconCss: 'e-icons e-back' }
            ]
        },
        {
            text: 'بحث البيانات',
            iconCss: 'em-icons e-edit',
            items: [
                { text: 'الملاحظات السابقة', iconCss: 'e-icons e-prev-notes' },
                { text: 'جدولة الملاحظات', iconCss: 'e-icons e-prev-notes'  },
                { text: 'بحث المرضى', iconCss: 'e-icons e-pat-search'  }
            ]
        },
        {
            text:  'الحالات المرضية',
            items: [
                { text: 'تعديل', iconCss: 'e-icons e-edit' },
                { text: 'عرض', iconCss: 'e-icons e-view'},
                { text: 'رفع ملفات', iconCss: 'e-icons e-upload-files' }
            ]
        },
        {
            text: 'التقارير',
            items: [
                { text: 'اصدار تقرير', iconCss: 'e-icons e-report-issue'},
                { text: 'حفظ تقرير', iconCss: 'e-icons e-report-save' },
                { text: 'تقرير قديم', iconCss: 'e-icons e-report-issue' }
            ]
        },
        
        {
            text: 'دفتر العناوين',
            items: [
                { text: 'اصدار تحويل', iconCss: 'e-icons e-report-issue' },
                { text: 'بحث تحويل', iconCss: 'e-icons e-report-issue' },
                { text: 'تقرير تحويل', iconCss: 'e-icons e-report-issue' }
            ]
        },
        {
            text: 'حركات النظام',
            items: [
                { text: 'حركات قديمة', iconCss: 'e-icons e-trans-follow'},
                { text: 'بحث التنفيذ', iconCss: 'e-icons e-trans-follow' },
                { text: 'متابعة الحالات', iconCss: 'e-icons e-trans-follow'}
            ]
        },
        {
            text: 'السجل',
            items: [
                { text: 'مكالمات قديمة', iconCss: 'e-icons e-calls-by' },
                { text: 'حسب القضية', iconCss: 'e-icons e-calls-by' },
                { text: 'حسب المدينة', iconCss: 'e-icons e-calls-by'},
                { text: 'حسب ادوات العنف', iconCss: 'e-icons e-calls-by' }
            ]
        },
        {
            text: 'بحث تحويل الشرطة',
            items: [
                { text: 'بحث الحالات المرضية', iconCss: 'e-icons e-police-case-edit' },
                { text: 'تعديل الاصدار', iconCss: 'e-icons e-police-case-edit'},
                { text: 'اضافة ملاحظات', iconCss: 'e-icons e-cases-police-notes' },
                { text: 'ارفاق ملفات', iconCss: 'e-icons e-cases-police-files' }
            ]
        },
        {
            text: 'قائمة النص',
            items: [
                { text: 'اغلاق الاداة' },
                { text: 'حفظ' }
            ]
        }
    ];
    try {
        var menuObj = new ej.navigations.Menu({ items: menuItems ,
            select: (args) => {
                if (args.item.parentObj.text) {
                    alert(args.item.parentObj.text + "/" + args.item.text);
                }
            }
        }, '#menu');
    } catch (e) {
        alert(e)
    }


}


var container;


function CreateDocumentEditor() {

    var hostUrl = 'http://localhost:62728/';
    var container = new ej.documenteditor.DocumentEditorContainer();
    ej.documenteditor.DocumentEditorContainer.Inject(ej.documenteditor.Toolbar);
    container.serviceUrl = hostUrl + 'api/documenteditor/';
    container.appendTo('#container');
    var defaultDocument = { "sections": [{ "sectionFormat": { "pageWidth": window.innerWidth - 200 , "pageHeight": window.innerHeight - 200, "leftMargin": 72, "rightMargin": 72, "topMargin": 72, "bottomMargin": 72, "differentFirstPage": false, "differentOddAndEvenPages": false, "headerDistance": 36, "footerDistance": 36 }, "blocks": [{ "paragraphFormat": { "afterSpacing": 30, "styleName": "Heading 1", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Adventure Works Cycles" }] }, { "paragraphFormat": { "firstLineIndent": 36, "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Adventure Works Cycles, the fictitious company on which the " }, { "characterFormat": {}, "text": "AdventureWorks" }, { "characterFormat": {}, "text": " sample databases " }, { "characterFormat": {}, "text": "are based, is a large, multinational manufacturing company. The company manufactures and sells metal " }, { "characterFormat": {}, "text": "and composite bicycles to North American, European and Asian commercial markets. While its base " }, { "characterFormat": {}, "text": "operation " }, { "characterFormat": {}, "text": "is located in" }, { "characterFormat": {}, "text": " Bothell, Washington with 290 employees, several regional sales teams are located " }, { "characterFormat": {}, "text": "throughout their market base." }] }, { "paragraphFormat": { "firstLineIndent": 36, "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "In 2000, Adventure Works Cycles bought a small manufacturing plant, " }, { "characterFormat": {}, "text": "Importadores" }, { "characterFormat": {}, "text": " Neptuno, " }, { "characterFormat": {}, "text": "located in Mexico. " }, { "characterFormat": {}, "text": "Importadores" }, { "characterFormat": {}, "text": " Neptuno manufactures several critical subcomponents for the " }, { "characterFormat": {}, "text": "Adventure Works Cycles product line. These subcomponents are shipped to the Bothell location for final " }, { "characterFormat": {}, "text": "product assembly. In 2001, " }, { "characterFormat": {}, "text": "Importadores" }, { "characterFormat": {}, "text": " Neptuno, became the sole manufacturer and distributor of the " }, { "characterFormat": {}, "text": "touring bicycle product group." }] }, { "paragraphFormat": { "styleName": "Heading 1", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product Overview" }] }, { "rows": [{ "cells": [{ "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": { "fontSize": "11" }, "inlines": [{ "characterFormat": { "fontSize": "11" }, "imageString": "data:image/gif;base64,", "width": 141.99998, "height": 88.5 }] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 0 }, { "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Heading 2", "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Mountai" }, { "characterFormat": {}, "bookmarkType": 0, "name": "_GoBack" }, { "characterFormat": {}, "bookmarkType": 1, "name": "_GoBack" }, { "characterFormat": {}, "text": "n-200" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product No: BK-M68B-38" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Size: 38" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Weight: 25" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Price: $2,294.99" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 1 }], "rowFormat": { "height": 1, "allowBreakAcrossPages": true, "heightType": "AtLeast", "isHeader": false, "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "gridBefore": 0, "gridBeforeWidth": 0, "gridBeforeWidthType": "Point", "gridAfter": 0, "gridAfterWidth": 0, "gridAfterWidthType": "Point" } }, { "cells": [{ "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Heading 2", "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Mountain-300" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product No: BK-M47B-38" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Size: 35" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Weight: 22" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Price: $1,079.99" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 0 }, { "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "imageString": "data:image/gif;base64,", "width": 135, "height": 84 }] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 1 }], "rowFormat": { "height": 1, "allowBreakAcrossPages": true, "heightType": "AtLeast", "isHeader": false, "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "gridBefore": 0, "gridBeforeWidth": 0, "gridBeforeWidthType": "Point", "gridAfter": 0, "gridAfterWidth": 0, "gridAfterWidthType": "Point" } }, { "cells": [{ "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "imageString": "data:image/gif;base64,", "width": 165.5, "height": 103 }] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 0 }, { "blocks": [{ "paragraphFormat": { "textAlignment": "Center", "styleName": "Heading 2", "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": { "fontSize": "13" }, "inlines": [{ "characterFormat": {}, "text": "Road-150" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Product No: BK-R93R-44" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Size: 44" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Weight: 14" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [{ "characterFormat": {}, "text": "Price: $3,578.27" }] }, { "paragraphFormat": { "textAlignment": "Center", "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "cellFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "topMargin": 2, "preferredWidth": 234, "preferredWidthType": "Point", "cellWidth": 234, "columnSpan": 1, "rowSpan": 1, "verticalAlignment": "Top" }, "columnIndex": 1 }], "rowFormat": { "height": 1, "allowBreakAcrossPages": true, "heightType": "AtLeast", "isHeader": false, "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "None", "lineWidth": 0, "shadow": false, "space": 0 } }, "gridBefore": 0, "gridBeforeWidth": 0, "gridBeforeWidthType": "Point", "gridAfter": 0, "gridAfterWidth": 0, "gridAfterWidthType": "Point" } }], "grid": [234, 234], "tableFormat": { "borders": { "top": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "left": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "right": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "bottom": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "diagonalDown": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "diagonalUp": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "horizontal": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 }, "vertical": { "color": "#000000", "hasNoneStyle": false, "lineStyle": "Single", "lineWidth": 0.5, "shadow": false, "space": 0 } }, "shading": { "backgroundColor": "#ffffff", "foregroundColor": "empty", "textureStyle": "TextureNone" }, "cellSpacing": 0, "leftIndent": 0, "tableAlignment": "Left", "topMargin": 0, "rightMargin": 5.4, "leftMargin": 5.4, "bottomMargin": 0, "preferredWidth": 0, "preferredWidthType": "Auto" }, "description": null, "title": null }, { "paragraphFormat": { "styleName": "Normal", "listFormat": {} }, "characterFormat": {}, "inlines": [] }], "headersFooters": { "header": { "blocks": [{ "paragraphFormat": { "listFormat": {} }, "characterFormat": {}, "inlines": [] }] }, "footer": { "blocks": [{ "paragraphFormat": { "listFormat": {} }, "characterFormat": {}, "inlines": [] }] } } }], "characterFormat": { "fontSize": 11, "fontFamily": "Calibri" }, "paragraphFormat": { "lineSpacing": 1.0791666507720947, "lineSpacingType": "Multiple", "listFormat": {} }, "styles": [{ "name": "Normal", "type": "Paragraph", "paragraphFormat": { "lineSpacing": 1.149999976158142, "lineSpacingType": "Multiple", "listFormat": {} }, "characterFormat": { "fontFamily": "Calibri" }, "next": "Normal" }, { "name": "Default Paragraph Font", "type": "Character", "characterFormat": {} }, { "name": "Heading 1 Char", "type": "Character", "characterFormat": { "fontSize": 16, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Default Paragraph Font" }, { "name": "Heading 1", "type": "Paragraph", "paragraphFormat": { "beforeSpacing": 12, "afterSpacing": 0, "outlineLevel": "Level1", "listFormat": {} }, "characterFormat": { "fontSize": 16, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Normal", "link": "Heading 1 Char", "next": "Normal" }, { "name": "Heading 2 Char", "type": "Character", "characterFormat": { "fontSize": 13, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Default Paragraph Font" }, { "name": "Heading 2", "type": "Paragraph", "paragraphFormat": { "beforeSpacing": 2, "afterSpacing": 6, "outlineLevel": "Level2", "listFormat": {} }, "characterFormat": { "fontSize": 13, "fontFamily": "Calibri Light", "fontColor": "#2F5496" }, "basedOn": "Normal", "link": "Heading 2 Char", "next": "Normal" }], "lists": [], "abstractLists": [] };
    var waitingPopUp = document.getElementById('waiting-popup');
    container.documentEditor.open(JSON.stringify(defaultDocument));
    container.documentEditor.documentName = 'Getting Started';
    // TitleBar sample starts
    titleBarDiv = document.getElementById('documenteditor_titlebar');
    initializeTitleBar(true);
    updateDocumentTitle();
    wireEventsInTitleBar();
    this.container = container;
    container.documentChange = function () {
        updateDocumentTitle();
    };
}



    var documentTitle;
    var documentTitleContentEditor;
    var titleBarDiv;
    var print;
    var openBtn;
    var download;
    var isPropertiesPaneEnabled;
    function initializeTitleBar(isShareNeeded) {
        documentTitle = ej.base.createElement('label', { id: 'documenteditor_title_name', styles: 'text-transform:capitalize;font-weight:400;text-overflow:ellipsis;white-space:pre;overflow:hidden;user-select:none;cursor:text' });
        documentTitleContentEditor = ej.base.createElement('div', { id: 'documenteditor_title_contentEditor', className: 'single-line' });
        documentTitleContentEditor.appendChild(documentTitle);
        titleBarDiv.appendChild(documentTitleContentEditor);
        documentTitleContentEditor.setAttribute('title', 'Document Name. Click or tap to rename this document.');
        var btnStyles = 'float:right;background: transparent;box-shadow:none; font-family: inherit;border-color: transparent;' +
            'border-radius: 2px;color:inherit;font-size:12px;text-transform:capitalize;margin-top:4px;height:28px;font-weight:400';
        print = addButton('e-de-icon-Print e-de-padding-right', 'Print', btnStyles, 'de-print', 'Print this document (Ctrl+P).', false);
        openBtn = addButton('e-de-icon-Open e-de-padding-right', 'open', btnStyles, 'de-open', 'Open', false);
        var items = [
            { text: 'Microsoft Word (.docx)', id: 'word' },
            { text: 'Syncfusion Document Text (.sfdt)', id: 'sfdt' },
        ];
        download = addButton('e-de-icon-Download e-de-padding-right', 'Download', btnStyles, 'documenteditor-share', 'Download this document.', true, items);
        if (!isShareNeeded) {
            download.element.style.display = 'none';
        }
        else {
            openBtn.element.style.display = 'none';
        }
}


function JqGridVoilance(extNo) {


    var source =
    {
        url: 'http://192.168.225.81:8090/api/pivotCallerSearchApi?peer=' + extNo,
        datatype: "json",
        type: 'GET',
        datafields:
            [
                { name: 'ContactMethodDetail', type: 'string' },
                { name: 'NickName', type: 'string' },
                { name: 'CreatedBy', type: 'string' },
                { name: 'StartTime', type: 'string' },
                { name: 'EndTime', type: 'string' },
                { name: 'Name', type: 'string' },
                { name: 'EmployeeName', type: 'string' },
                { name: 'DescriptionOfTrend', type: 'string' },
                { name: 'FollowUpDescription', type: 'string' }
            ]
    };

    var dataAdapter = new jqx.dataAdapter(source);
    var cellsrenderer = function (row, columnfield, value, defaulthtml, columnproperties, rowdata) {
        if (value != '' && value != null && value != undefined && value != 'undefined') {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #ff0000;">' + value + '</span>';
        }
        else {
            return '<span style="margin: 4px; margin-top:8px; float: ' + columnproperties.cellsalign + '; color: #008000;">' + value + '</span>';
        }
    }

    var initrowdetails = function (index, parentElement, gridElement, datarecord) {
        var tabsdiv = null;
        var information = null;
        var notes = null;
        tabsdiv = $($(parentElement).children()[0]);
        if (tabsdiv != null) {
            information = tabsdiv.find('.information');
            notes = tabsdiv.find('.notes');
            var title = tabsdiv.find('.title');
            title.text(datarecord.ContactMethodDetail);
            var container = $('<div style="margin: 5px;"></div>')
            container.appendTo($(information));
            var photocolumn = $('<div style="float: left; width: 15%;"></div>');
            var leftcolumn = $('<div style="float: left; width: 45%;"></div>');
            var rightcolumn = $('<div style="float: left; width: 40%;"></div>');
            /*container.append(photocolumn);*/
            container.append(leftcolumn);
            container.append(rightcolumn);
            /*var photo = $("<div class='jqx-rc-all' style='margin: 10px;'><b>Photo:</b></div>");
            var image = $("<div style='margin-top: 10px;'></div>");
            var imgurl = '../../images/' + datarecord.firstname.toLowerCase() + '.png';
            var img = $('<img height="60" src="' + imgurl + '"/>');*/
            /*image.append(img);
            image.appendTo(photo);
            photocolumn.append(photo);*/
            var descriptionOfTrend = "<div style='margin: 10px;'><b>EmployeeName:</b> " + datarecord.EmployeeName + "</div>";
            /*var lastname = "<div style='margin: 10px;'><b>Last Name:</b> " + datarecord.lastname + "</div>";
            var title = "<div style='margin: 10px;'><b>Title:</b> " + datarecord.title + "</div>";
            var address = "<div style='margin: 10px;'><b>Address:</b> " + datarecord.address + "</div>";*/
            $(leftcolumn).append(descriptionOfTrend);
            /*$(leftcolumn).append(lastname);
            $(leftcolumn).append(title);
            $(leftcolumn).append(address);*/
            var FollowUpDescription = "<div style='margin: 10px;'><b>Person Name:</b> " + datarecord.NickName + "</div>";
            /*var city = "<div style='margin: 10px;'><b>City:</b> " + datarecord.city + "</div>";
            var phone = "<div style='margin: 10px;'><b>Phone:</b> " + datarecord.homephone + "</div>";
            var hiredate = "<div style='margin: 10px;'><b>Hire Date:</b> " + datarecord.hiredate + "</div>";*/
            $(rightcolumn).append(FollowUpDescription);
            /*$(rightcolumn).append(city);
            $(rightcolumn).append(phone);
            $(rightcolumn).append(hiredate);*/
            var notescontainer = $('<div style="white-space: normal; margin: 5px;"><span>' + datarecord.DescriptionOfTrend + '</span></div>');
            $(notes).append(notescontainer);
            $(notes).append('</br> </hr style="color:red;">');
            var followup = $('<div style="white-space: normal; margin: 5px;"><span>' + datarecord.FollowUpDescription + '</span></div>');
            $(notes).append(followup);
            $(tabsdiv).jqxTabs({ width: '100%', height: 250 });
        }
    }
    $("#gridvoilance").jqxGrid(
        {
            width: '100%',
            source: dataAdapter,
            pageable: true,
            autoheight: true,
            sortable: true,
            altrows: true,
            enabletooltips: true,
            showtoolbar: true,
            rendertoolbar: function (statusbar) {
                // appends buttons to the status bar.
                var container = $("<div style='overflow: hidden; position: relative; margin: 5px;'></div>");
                var addButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/add.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var deleteButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-pdf-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var reloadButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-search-64.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                var searchButton = $("<div style='float: left; margin-left: 5px;'><img style='position: relative; margin-top: 2px;' width='20' height='18' src='assets/img/icons8-info-48.png'/><span style='margin-left: 4px; position: relative; top: -3px;'></span></div>");
                container.append(addButton);
                container.append(deleteButton);
                container.append(reloadButton);
                container.append(searchButton);
                statusbar.append(container);
                addButton.jqxButton({ width: 60, height: 20 });
                deleteButton.jqxButton({ width: 65, height: 20 });
                reloadButton.jqxButton({ width: 65, height: 20 });
                searchButton.jqxButton({ width: 50, height: 20 });
                // add new row.
                addButton.click(function (event) {
                    var datarow = generatedata(1);
                    $("#gridvoilance").jqxGrid('addrow', null, datarow[0]);
                });
                // delete selected row.
                deleteButton.click(function (event) {
                    var selectedrowindex = $("#grid").jqxGrid('getselectedrowindex');
                    var rowscount = $("#grid").jqxGrid('getdatainformation').rowscount;
                    var id = $("#grid").jqxGrid('getrowid', selectedrowindex);
                    $("#gridvoilance").jqxGrid('deleterow', id);
                });
                // reload grid data.
                reloadButton.click(function (event) {
                    $("#gridvoilance").jqxGrid({ source: getAdapter() });
                });
                // search for a record.
                searchButton.click(function (event) {
                    var offset = $("#gridvoilance").offset();
                    $("#jqxwindow").jqxWindow('open');
                    $("#jqxwindow").jqxWindow('move', offset.left + 30, offset.top + 30);
                });
            },
            editable: true,
            selectionmode: 'multiplecellsadvanced',
            rowdetails: true,
            rowdetailstemplate: { rowdetails: "<div style='margin: 10px;'><ul style='margin-left: 30px;'><li class='title'></li><li>Notes</li></ul><div class='information'></div><div class='notes'></div></div>", rowdetailsheight: 200 },
            initrowdetails: initrowdetails,
            ready: function () {
                $("#gridvoilance").jqxGrid('showrowdetails', 0);
                $("#gridvoilance").jqxGrid('showrowdetails', 1);
            },
            columns: [
                { text: 'ContactMethodDetail', columngroup: 'ContactMethodDetail', datafield: 'ContactMethodDetail', width: '13%' },
                { text: 'NickName', columngroup: 'ContactMethodDetail', datafield: 'NickName', cellsalign: 'right', align: 'right', width: '13%' },
                { text: 'CreatedBy', columngroup: 'EmployeeName', datafield: 'CreatedBy', align: 'right', cellsalign: 'right', cellsformat: 'c2', width: '13%' },
                { text: 'StartTime', datafield: 'StartTime', cellsalign: 'StartTime', cellsrenderer: cellsrenderer, width: '13%' },
                { text: 'EndTime', datafield: 'EndTime', cellsalign: 'EndTime', cellsrenderer: cellsrenderer, width: '13%' },
                { text: 'Name', datafield: 'Name', cellsalign: 'Name', cellsrenderer: cellsrenderer, width: '13%' },
                { text: 'EmployeeName', datafield: 'EmployeeName', cellsalign: 'EmployeeName', cellsrenderer: cellsrenderer, width: '13%' }
            ],
            columngroups: [
                { text: 'ContactMethodDetail', align: 'center', name: 'ContactMethodDetail' },
                { text: 'EmployeeName', align: 'center', name: 'EmployeeName' }
            ]
        });

}

    function wireEventsInTitleBar() {
        print.element.addEventListener('click', onPrint);
        openBtn.element.addEventListener('click', function (e) {
            if (e.target.id === 'de-open') {
                var fileUpload = document.getElementById('uploadfileButton');
                fileUpload.value = '';
                fileUpload.click();
            }
        });
        documentTitleContentEditor.addEventListener('keydown', function (e) {
            if (e.keyCode === 13) {
                e.preventDefault();
                documentTitleContentEditor.contentEditable = 'false';
                if (documentTitleContentEditor.textContent === '') {
                    documentTitleContentEditor.textContent = 'Document1';
                }
            }
        });
        documentTitleContentEditor.addEventListener('blur', function () {
            if (documentTitleContentEditor.textContent === '') {
                documentTitleContentEditor.textContent = 'Document1';
            }
            documentTitleContentEditor.contentEditable = 'false';
            container.documentEditor.documentName = documentTitle.textContent;
        });
        documentTitleContentEditor.addEventListener('click', function () {
            updateDocumentEditorTitle();
        });
    }
    function updateDocumentEditorTitle() {
        documentTitleContentEditor.contentEditable = 'true';
        documentTitleContentEditor.focus();
        window.getSelection().selectAllChildren(documentTitleContentEditor);
    }
    function updateDocumentTitle() {
        if (container.documentEditor.documentName === '') {
            container.documentEditor.documentName = 'Untitled';
        }
        documentTitle.textContent = container.documentEditor.documentName;
    }
    function onPrint() {
        container.documentEditor.print();
    }
    function onExportClick(args) {
        var value = args.item.id;
        switch (value) {
            case 'word':
                save('Docx');
                break;
            case 'sfdt':
                save('Sfdt');
                break;
        }
    }
    function save(format) {
        container.documentEditor.save(container.documentEditor.documentName === '' ? 'sample' : container.documentEditor.documentName, format);
    }
    function setTooltipForPopup() {
        document.getElementById('documenteditor-share-popup').querySelectorAll('li')[0].setAttribute('title', 'Download a copy of this document to your computer as a DOCX file.');
        document.getElementById('documenteditor-share-popup').querySelectorAll('li')[1].setAttribute('title', 'Download a copy of this document to your computer as an SFDT file.');
    }
    function addButton(iconClass, btnText, styles, id, tooltipText, isDropDown, items) {
        var button = ej.base.createElement('button', { id: id, styles: styles });
        titleBarDiv.appendChild(button);
        button.setAttribute('title', tooltipText);
        if (isDropDown) {
            var dropButton = new ej.splitbuttons.DropDownButton({ select: onExportClick, items: items, iconCss: iconClass, cssClass: 'e-caret-hide', content: btnText, open: function () { setTooltipForPopup(); } }, button);
            return dropButton;
        }
        else {
            var ejButton = new ej.buttons.Button({ iconCss: iconClass, content: btnText }, button);
            return ejButton;
        }
    } 


function CloseEditorSyncFusion() {
    this.container.documentEditor.close();
}


